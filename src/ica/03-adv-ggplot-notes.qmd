...
how much warmer their locale was in 2015 relative to historical expectations. The focus is on the specific region itself. 

  - How did this goal inform what information is displayed in the visualization?
  The visualizaiton shows the actual high and low temperature compare to the normal temperature range and the record high and low for each day in 2015. 

**Aesthetic Mapping** 

- What specific variables (from the data codebook) underlie the visualization?
> date, low, high, normalhigh, normallow, recordhigh, recordlow

- How do these variables map to aesthetics of the visual elements, eg, position, size, shape, and color of glyphs?
> Position: x-axis: Month, Day(time sequence through the year)
y axis: temperature

> vertical bars: light gray for record low to high, gray: normal low to high, red: actual low to high. These bars may be color-coded and layered or grouped to visually compare actual vs. expected vs. extreme ranges.

> glyphs: One days with new record highs or lows, discrete colored dots (eg. red for new high, blue for new low) appear on the bar chart to highlight those extreme days. 
...
weather_clean <- weather |>
  mutate(month_name = month(date, label = FALSE, abbr = TRUE))
weather_first <- weather|>
  group_by(Month) |>
  filter(Day == "1")
weather_Text <- weather |>
  filter(RecordText != "NA")

```

```{r }
weather_plot <- ggplot(weather_clean) +
  geom_linerange(aes(x = dateInYear, ymin = RecordLow, ymax = RecordHigh), color = "#ECEBE3")+
  geom_linerange(aes(x = dateInYear,ymin = NormalLow, ymax = NormalHigh), color = "#C8B8BA")+
  geom_linerange(aes(x = dateInYear,ymin = Low, ymax = High), color = "#A90248")+
  scale_x_continuous(breaks = seq(15, 365, by=30), labels = month.abb, expand = c(0,0))+
  geom_text_repel(data = weather_Text, aes(x = dateInYear, y = RecordHigh, label = RecordText, force = 2, nudge_y = 200))+
  geom_point(data = weather_Text, aes(x = dateInYear, y = RecordHigh, label = RecordText), shape = 25, fill = "red", size = 5)+
  geom_vline(data = weather_first, aes(xintercept = dateInYear), linetype = "dashed")
    theme_classic()
weather_plot
```
