...
Yes, `str()` does this plus more for you.
Let's consider replicating this task through low-level iteration.
...
`map_chr()` was expecting to create a character vector with one element per element (column) in `diamonds`.
But something happened in column 2 with the `cut` variable.
...
Ah!
`cut` has 2 classes.
In this case, `map()` (which returns a list) is the best option because some variables have multiple classes:
...
Recall that we explored syntax and functions for selecting variables in a data frame via the tidy-select documentation for `across`.
(`?dplyr_tidy_select` in the Console.)
...
Once you've done that with `map()`, write a `for` loop to achieve the same result.
Which do you prefer in terms of ease of code writing and readability?
...
It converts/unfolds the information into a numeric vector (repeating the information in `mean`, `sd`, `n`)
...
-   [Functionals Chapter in Advanced R](https://adv-r.hadley.nz/functionals.html) by Hadley Wickham \## Done!
