...
1. **Missing data mechanism** For each of the following situations, propose what missing data mechanism you think is most likely at play. 
  ...
  > Could be MCAR if the missing values are from technical glitches. Could be MAR if the missing values are related to other measured weather variables. Could be MNAR if the values tend to be missing when the values themselves are high(eg. high temperature, humidity, and wind speed causing measurement devices to malfunction)
