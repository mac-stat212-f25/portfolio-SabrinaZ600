...
>_d() variant is used when color is mapped to a discrete(categorical) variable
_c() variant is used when color is mapped to a continous variable
-b() variant is used when color is mapped to a continous variable but we want the variable binned. 
...
png("relative path to image", width = 800, height = 400)
